        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          <div className="lg:col-span-1">
            <div className="bg-surface-800/40 border border-surface-700/50 rounded-xl overflow-hidden shadow-lg">
              ...
              {/* Header Row */}
              <div className="grid grid-cols-[32px_1fr_repeat(5,_minmax(30px,42px))] items-center px-4 py-2 border-b border-surface-700/50 bg-surface-900/30">
                <div className="text-[9px] font-black text-surface-600 uppercase tracking-widest">#</div>
              ...
              <div className="divide-y divide-surface-700/50 max-h-[600px] overflow-y-auto no-scrollbar">
                {teamScores.length > 0 ? teamScores.map((team) => {
                  ...
                  return (
                    ...
                    >
                      <div className="grid grid-cols-[32px_1fr_repeat(5,_minmax(30px,42px))] items-center px-4 py-3">
                        <div className={`w-6 h-6 rounded-lg border flex items-center justify-center text-[10px] font-black transition-colors ${
                          isSelected 
                        ...
                        <div className="pl-2 min-w-0">
                          <div className={`font-bold text-xs md:text-sm transition-colors truncate ${
                            isSelected ? 'text-primary-400' : 'text-surface-100 group-hover:text-primary-400'
                          }`}>{team.team_name}</div>
                          <div className="text-[9px] text-surface-500 hidden md:block">
                            {team.golfer_scores.length > 0 
                              ? `${team.golfer_scores.length} starters` 
                              : 'No lineup set'}
                          </div>
                        </div>
                        <div className={`text-center text-[10px] md:text-xs font-bold ${scoreColor(team.r1)}`}>{formatScore(team.r1)}</div>
                        <div className={`text-center text-[10px] md:text-xs font-bold ${scoreColor(team.r2)}`}>{formatScore(team.r2)}</div>
                        <div className={`text-center text-[10px] md:text-xs font-bold ${scoreColor(team.r3)}`}>{formatScore(team.r3)}</div>
                        <div className={`text-center text-[10px] md:text-xs font-bold ${scoreColor(team.r4)}`}>{formatScore(team.r4)}</div>
                        <div className={`text-center text-[10px] md:text-sm font-black font-display ${scoreColor(team.total)}`}>
          ...
          <div className="lg:col-span-1">
            <div className="bg-surface-800/40 border border-surface-700/50 rounded-xl overflow-hidden shadow-lg">
              ...
              {/* Header Row */}
              <div className="grid grid-cols-[32px_1fr_repeat(5,_minmax(30px,42px))_20px] items-center px-4 py-2 border-b border-surface-700/50 bg-surface-900/30">
                <div className="text-[9px] font-black text-surface-600 uppercase tracking-widest">#</div>
                ...
                {displayGolfers.length > 0 ? displayGolfers.map((golfer) => {
                  ...
                  return (
                    <div key={golfer.golfer_id} className="group">
                      ...
                      >
                        {/* Main Info Row */}
                        <div className="grid grid-cols-[32px_1fr_repeat(5,_minmax(30px,42px))_20px] items-center px-4 py-3">
                          <div className="w-6 h-6 rounded-lg bg-surface-900 border border-surface-700 flex items-center justify-center text-[10px] font-black text-surface-500 group-hover:border-primary-500/30 group-hover:text-primary-400 transition-colors">
                            {golfer.displayRank}
                          </div>
                          <div className="pl-2 min-w-0">
                          ...
                          {/* Round Scores */}
                          <div className={`text-center text-[10px] md:text-xs font-bold ${scoreColor(golfer.r1)}`}>{formatScore(golfer.r1)}</div>
                          <div className={`text-center text-[10px] md:text-xs font-bold ${scoreColor(golfer.r2)}`}>{formatScore(golfer.r2)}</div>
                          <div className={`text-center text-[10px] md:text-xs font-bold ${golfer.is_penalty ? 'text-red-400/60 italic' : scoreColor(golfer.r3)}`}>
                            {formatScore(golfer.r3)}{golfer.is_penalty ? '*' : ''}
                          </div>
                          <div className={`text-center text-[10px] md:text-xs font-bold ${golfer.is_penalty ? 'text-red-400/60 italic' : scoreColor(golfer.r4)}`}>
                            {formatScore(golfer.r4)}{golfer.is_penalty ? '*' : ''}
                          </div>

                          {/* Total Score */}
                          <div className={`text-center text-[10px] md:text-sm font-black font-display ${scoreColor(golfer.total)}`}>
                            {formatScore(golfer.total)}
                          </div>

                          {/* Expand Arrow */}
                          <div className="w-5 flex items-center justify-center text-surface-600 group-hover:text-surface-400 transition-colors">
                            <span className={`text-[10px] transition-transform duration-200 ${expandedGolfer === golfer.golfer_id ? 'rotate-180' : ''}`}>▼</span>
                          </div>
                        </div>
                      </div>
